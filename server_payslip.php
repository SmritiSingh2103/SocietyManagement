...
$esi = "";
	...
	if(isset($_POST['search_by_name'])){
		if(!empty($_POST['firstname']) && !empty($_POST['lastname'])){			
			...
			if($no == 1){
				...
				$Error = "*$no $fn $ln exist...Please search by Id()";
			}
			else{
	...
	if(isset($_POST['generate'])){
			...
			$pdf->SetXY(95, 05);
			$pdf->Cell(300,03,'Department Store');
			$pdf->SetFont('Arial','', 8 );
			$pdf->SetXY(88, 10);

			$pdf->Cell(03,03,'     JM Road,','centered');
			$pdf->SetFont('Arial','', 8 );
			$pdf->SetXY(88, 16);
			$pdf->Cell(03,03,'           Pune-411016.','centered');
			$pdf->SetFont('Arial','', 8 );
			$pdf->SetXY(88, 19);
			$pdf->Cell(03,03,' Contact No:9943255934','centered');
			$pdf->SetFont('Arial','', 10 );
			$pdf->SetXY(86, 24);
			$pdf->Cell(03,03,'Payslip','centered');
			$pdf->Line(0, 30, 230, 30);
			$pdf->SetFont('Arial','B', 8 );
			$pdf->SetXY(05, 35);
			$pdf->Cell(03,03,'Bill ID:','centered');
			$tempid = $_POST['billno'];
			$pdf->SetFont('Arial','', 8 );
			$pdf->SetXY(30, 36.5);
			$pdf->Write(0,$tempid);
			...
			$date = $_POST['date'];
			$sql = "SELECT receipt.product_id,product.name,product.brand,product.category,product.amount,product.discount,receipt.quantity,receipt.total_price FROM product,receipt WHERE receipt.bill_id LIKE '$billno' AND product.product_id = receipt.product_id";
			$result = mysqli_query($con,$sql);
			//$sql = "select A.id,firstname,lastname,department,designation,salary,doj,overtime from employee as A, job as B, attendance as C where A.id = '$emp_id' and B.id = A.id and C.id = A.id  ";
			//$result = mysqli_query($con,$sql);

			$row = $result->fetch_assoc();
			$emp_id = $row['productid'];
			$fn = $row['name'];			
			$ln = $row['brand'];	
			$dept = $row['amount'];
			$desg = $row['discount'];
			$bsal = $row['total_price'];
			...
			$pdf->SetFont('Arial','B', 8 );
			$pdf->SetXY(05, 38);
			$pdf->Cell(03,03,'Product ID:','centered');
